...
#if USE(WRATH)
        // Excess copying of the QPixmap object, but I don't know how to avoid it...
        NativeImageWRATH nativeImage(new QPixmap(instance->toPixmap()));
        PassRefPtr<StillImage> stillImage = WebCore::StillImage::create(nativeImage); 
#else
...
#if USE(WRATH)
        QPixmap* pixmap = image->nativeImageForCurrentFrame()->getPixmap();
#else
