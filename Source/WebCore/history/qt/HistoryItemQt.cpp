...
bool WebCore::HistoryItem::restoreState(QDataStream& in, int version)
{
    ...
    in >> smoothZoom >> smoothScrollX >> smoothScrollY;
    setSmoothZoom(smoothZoom);
    setSmoothScroll(FloatPoint(smoothScrollX, smoothScrollY));
#endif


    in >> dailyVisitCounts >> weeklyVisitCounts;
    /*in >> loadFormdata;
    if (loadFormdata) {
...
#if USE(WRATH)
    FloatPoint p(getSmoothScroll());
    out << getSmoothZoom() << p.x() << p.y();
#endif
