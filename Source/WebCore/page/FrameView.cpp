...
namespace
{
  ...
  class FrameView_WRATHWidget:
    public WebCore::PaintedWidgetsOfWRATHT<WebCore::ScrollView, FrameView_WRATHWidget>
  {
  public:
    WebCore::PaintedWidgetsOfWRATHHandleT<WebCore::RenderLayer> m_render_layer;
    WebCore::PaintedWidgetsOfWRATHHandleT<WebCore::RenderLayer> m_overlay_scrollbars;
  };
...
void FrameView::readyWRATHWidgetsContentsImplement(ContextOfWRATH *ctx,
                                                   PaintedWidgetsOfWRATHHandleT<ScrollView> &hnd,
                                                   const IntRect &region)
{
  FrameView_WRATHWidget *d;
  d=FrameView_WRATHWidget::object(this, hnd);

  ContextOfWRATH::AutoPushNode autoPushRoot(ctx, d->m_root_node);
  d->m_render_layer.visible(false);
  d->m_overlay_scrollbars.visible(false);
  ...
  if (!contentRenderer) {
    LOG_ERROR("called FrameView::readWRATHWidgetsContents with nil renderer");
    return;
  }
  ...
  d->m_render_layer.visible(true);

  rootLayer->readyWRATHWidgets(ctx, d->m_render_layer, rect, m_paintBehavior);

  if (rootLayer->containsDirtyOverlayScrollbars()) {
    d->m_overlay_scrollbars.visible(true);
    rootLayer->readyWRATHOverlayScrollbars(ctx, d->m_overlay_scrollbars, rect, m_paintBehavior, eltRenderer);
  }
