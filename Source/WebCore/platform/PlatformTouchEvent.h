...
public:
    ...
    PlatformTouchEvent &applyTransformation(const ApplyTransformationToPoint &op)
    {
      for(Vector<PlatformTouchPoint>::iterator iter=m_touchPoints.begin(),
            end=m_touchPoints.end(); iter!=end; ++iter)
        {
          iter->applyTransformation(op);
        }
      return *this;
    }
