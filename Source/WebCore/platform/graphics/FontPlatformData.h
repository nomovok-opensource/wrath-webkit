...
/*
 [WRATH-DANGER] the "actual" font structure of the platform 
 is stored in FontPlatformData, thus when one puts WRATHWebKit
 on a platform where one also wishes to keep traditional drawing,
 then that platform's FontPlatformData srtuct also needs to
 store a pointer to the WRATH font and return it with the
 function 

  WRATHTextureFont *fontOfWRATH(void) const
 */
