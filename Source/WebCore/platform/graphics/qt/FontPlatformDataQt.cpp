...
#if USE(WRATH)
    m_data->m_fontOfWRATH=other.m_data->m_fontOfWRATH;
#endif
}
...
{ 
  /*
    We need to get the bytes from the rawFont and
    make a WRATHFontDatabase::FontMemorySource
    for the data.
   */
}
...
FontPlatformData::FontPlatformData(const QFont& font)
...
#if USE(WRATH)
FontPlatformData::FontPlatformData(const QFont& font, const WRATHFontDatabase::Font::const_handle &fnt)
  : m_data(adoptRef(new FontPlatformDataPrivate(font)))
{ 
  m_data->m_fontOfWRATH=WRATHFontFetch::fetch_font(fnt);
}
#endif
...
#if USE(WRATH)
static WRATHTextureFont *defaultWRATHFont(void)
{
  return WRATHFontFetch::fetch_font(WRATHFontFetch::FontProperties()
				    .family_name("Sans"));

}


WRATHTextureFont *FontPlatformData::fontOfWRATH() const
{
  if(m_data) {
    if(!m_data->m_fontOfWRATH) {
      
      WRATHFontFetch::FontProperties chooser;
      WRATHQT::generate_font_properties(font(), chooser);
      
      m_data->m_fontOfWRATH=WRATHFontFetch::fetch_font(chooser);
    }
    return m_data->m_fontOfWRATH;
  }

  return defaultWRATHFont();
