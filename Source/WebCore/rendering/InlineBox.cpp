...
{
    if (!determinedIfNextOnLineExists()) {
        setDeterminedIfNextOnLineExists();
...
{
    if (!determinedIfPrevOnLineExists()) {
        setDeterminedIfPrevOnLineExists();
        
        if (!parent())
            setPrevOnLineExists(false);
        else if (prevOnLine())
            setPrevOnLineExists(true);
        else
            setPrevOnLineExists(parent()->prevOnLineExists());
    }
...
bool InlineBox::canAccommodateEllipsis(bool ltr, int blockEdge, int ellipsisWidth)
{
    ...
    IntRect boxRect(x(), 0, logicalWidth(), 10);
