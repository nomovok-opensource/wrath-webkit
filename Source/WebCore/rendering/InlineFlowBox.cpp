...
void InlineFlowBox::addToLine(InlineBox* child) 
{
    ...
    if (!m_firstChild) {
        m_firstChild = child;
#if USE(WRATH)
        m_signal(E_firstChild);
#endif
        m_lastChild = child;
#if USE(WRATH)
        m_signal(E_lastChild);
#endif
    } else {
...
bool InlineFlowBox::nodeAtPoint(const HitTestRequest& request, HitTestResult& result, int ax, int ay, int tx, int ty, int lineTop, int lineBottom)
{
    IntRect overflowRect(visualOverflowRect(lineTop, lineBottom));
    flipForWritingMode(overflowRect);
    overflowRect.move(tx, ty);
    if (!overflowRect.intersects(result.rectForPoint(ax, ay)))
        return false;

    // Check children first.
    for (InlineBox* curr = lastChild(); curr; curr = curr->prevOnLine()) {
        if ((curr->renderer()->isText() || !curr->boxModelObject()->hasSelfPaintingLayer()) && curr->nodeAtPoint(request, result, ax, ay, tx, ty, lineTop, lineBottom)) {
    ...
    if (visibleToHitTesting() && rect.intersects(result.rectForPoint(ax, ay))) {
        renderer()->updateHitTestResult(result, flipForWritingMode(IntPoint(ax - tx, ay - ty))); // Don't add in x() or y() here, we want coords in the containing block's space.
        if (!result.addNodeToRectBasedTestResult(renderer()->node(), ax, ay, rect))
            return true;
    }
