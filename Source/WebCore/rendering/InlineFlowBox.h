...
public:
    ...
    enum MemberChange
    {
        ...
        E_nextLineBox,
        E_prevLineBox,
        E_includeLogicalEdges,
        E_descendantsHaveSameLineHeightAndBaseLine,
        E_hasTextChildren,
        E_hasTextDescendants,
        E_numberOfChanges
    };
    typedef Signal<InlineFlowBox> SignalType;
...
#endif
    {
        ...
        setHasTextChildren(obj->style()->display() == LIST_ITEM);
        setHasTextDescendants(hasTextChildren());
    }
    ...
    void setNextLineBox(InlineFlowBox* n) { SET_VALUE_AND_EMIT(m_nextLineBox, n, m_signal, E_nextLineBox); }
    void setPreviousLineBox(InlineFlowBox* p) { SET_VALUE_AND_EMIT(m_prevLineBox, p, m_signal, E_prevLineBox); }
...
#if USE(WRATH)
  virtual void readyWRATHWidgets(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                 PaintInfoOfWRATH&, int tx, int ty, int lineTop, int lineBottom);

  virtual void readyWRATHWidgetBoxDecorations(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                              PaintInfoOfWRATH&, int tx, int ty);

  virtual void readyWRATHWidgetMask(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                    PaintInfoOfWRATH&, int tx, int ty);

  void readyWRATHWidgetFillLayers(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                  const PaintInfoOfWRATH&, const Color&, const FillLayer*, 
                                  int tx, int ty, int w, int h, CompositeOperator = CompositeSourceOver);

  void readyWRATHWidgetFillLayer(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                 const PaintInfoOfWRATH&, const Color&, const FillLayer*, 
                                 int tx, int ty, int w, int h, CompositeOperator = CompositeSourceOver);

  void readyWRATHWidgetBoxShadow(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                 ContextOfWRATH*, RenderStyle*, ShadowStyle, int tx, int ty, int w, int h);
    ...
    { 
        if(m_descendantsHaveSameLineHeightAndBaseline)
        {
            m_descendantsHaveSameLineHeightAndBaseline = false;
#if USE(WRATH)
            m_signal(E_descendantsHaveSameLineHeightAndBaseLine);
#endif
        }
...
private:
    ...
    void setHasTextChildren(bool b=true)
    {
        if(m_hasTextChildren != b)
        {
            SET_VALUE_AND_EMIT(m_hasTextChildren, b, m_signal, E_hasTextChildren);
        }
    }

    void setHasTextDescendants(bool b=true)
    {
        if(m_hasTextDescendants != b)
        {
            SET_VALUE_AND_EMIT(m_hasTextDescendants, b, m_signal, E_hasTextDescendants);
        }
    }
...
#if USE(WRATH)
    void readyWRATHWidgetFillLayersWalk(PaintedWidgetsOfWRATHHandleT<InlineBox> &handle, 
                                        const PaintInfoOfWRATH&, const Color&, const FillLayer*, 
                                        int tx, int ty, int w, int h, CompositeOperator = CompositeSourceOver);
    SignalType m_signal;
#endif
