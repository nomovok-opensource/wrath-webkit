...
int InlineTextBox::lineHeight() const
{
    if (!isText() || !renderer()->parent())
        return 0;
    if (renderer()->isBR())
        return toRenderBR(renderer())->lineHeight(m_firstLine);
    if (parent()->renderer() == renderer()->parent())
...
{
    int sPos = max<int>(startPos - start(), 0);
    int ePos = min<int>(endPos - start(), (int)len());
    return (sPos < ePos);
}
...
RenderObject::SelectionState InlineTextBox::selectionState()
{
    ...
    if (state == RenderObject::SelectionStart || state == RenderObject::SelectionEnd || state == RenderObject::SelectionBoth) {
        ...
        bool isStart = (state != RenderObject::SelectionEnd && startPos >= start() && startPos < start() + len());
        bool end = (state != RenderObject::SelectionStart && endPos > start() && endPos <= lastSelectable);
        if (isStart && end)
        ...
        else if ((state == RenderObject::SelectionEnd || startPos < start()) &&
    ...
    if (truncation() != cNoTruncation && root()->ellipsisBox()) {
        ...
        if (state != RenderObject::SelectionNone) {
            ...
            // truncation.
            ellipsis->setSelectionState(end >= truncation() && start <= truncation() ?
...
IntRect InlineTextBox::selectionRect(int tx, int ty, int startPos, int endPos)
{
    ...
    const UChar* characters = textObj->text()->characters() + start();
    int length = len();
    ...
    IntRect r = enclosingIntRect(f.selectionRectForText(TextRun(characters, length, textObj->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride()),
...
float InlineTextBox::placeEllipsisBox(bool flowIsLTR, float visibleLeftEdge, float visibleRightEdge, float ellipsisWidth, bool& foundBox)
{
    ...
    bool rtlFullTruncation = !flowIsLTR && ellipsisX >= (x() + logicalWidth());
    ...
    bool ltrEllipsisWithinBox = flowIsLTR && (ellipsisX < x() + logicalWidth());
    ...
    if (ltrEllipsisWithinBox || rtlEllipsisWithinBox) {
        ...
        if (ltr != flowIsLTR) {
          ...
          ellipsisX = ltr ? x() + visibleBoxWidth : x() + logicalWidth() - visibleBoxWidth;
        }
        ...
        float widthOfVisibleText = toRenderText(renderer())->width(start(), offset, textPos(), m_firstLine);
        ...
        if (flowIsLTR)
            ...
            return (x() + logicalWidth()) - widthOfVisibleText - ellipsisWidth;
    }
    return -1;
}
...
bool InlineTextBox::nodeAtPoint(const HitTestRequest&, HitTestResult& result, int x, int y, int tx, int ty, int /* lineTop */, int /*lineBottom*/)
{
    ...
    if (truncation() != cFullTruncation && visibleToHitTesting() && rect.intersects(result.rectForPoint(x, y))) {
...
void InlineTextBox::paint(PaintInfo& paintInfo, int tx, int ty, int /*lineTop*/, int /*lineBottom*/)
{
    ...
    TextRun textRun(characters, length, textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride() || styleToUse->visuallyOrdered());
...
void InlineTextBox::selectionStartEnd(int& sPos, int& ePos)
{
    ...
    sPos = max<int>(startPos - start(), 0);
    ePos = min<int>(endPos - start(), (int)len());
}
...
void InlineTextBox::paintSelection(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font)
{
    ...
    context->drawHighlightForText(font, TextRun(characters, length, textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), 
...
void InlineTextBox::paintCompositionBackground(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font, int startPos, int endPos)
{
    ...
    int ePos = min(endPos - offset, (int)len());
    ...
    int selHeight = selectionHeight();
    FloatPoint localOrigin(boxOrigin.x(), boxOrigin.y() - deltaY);
    context->drawHighlightForText(font, TextRun(textRenderer()->text()->characters() + start(), len(), textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(),
                                  direction(), dirOverride() || style->visuallyOrdered()),
                                  localOrigin, selHeight, c, style->colorSpace(), sPos, ePos);
}
...
void InlineTextBox::paintDecoration(GraphicsContext* context, const FloatPoint& boxOrigin, 
                                    int deco, const ShadowData* shadow)
{
...
void InlineTextBox::paintSpellingOrGrammarMarker(GraphicsContext* pt, const FloatPoint& boxOrigin, const DocumentMarker& marker, RenderStyle* style, const Font& font, bool grammar)
{
    ...
    if (start() <= (int)marker.startOffset)
    ...
    if (!markerSpansWholeBox || grammar) {
        int startPosition = max<int>(marker.startOffset - start(), 0);
        int endPosition = min<int>(marker.endOffset - start(), len());
        
        if (truncation() != cNoTruncation)
            endPosition = min<int>(endPosition, truncation());

        // Calculate start & width
        int deltaY = renderer()->style()->isFlippedLinesWritingMode() ? selectionBottom() - logicalBottom() : logicalTop() - selectionTop();
        int selHeight = selectionHeight();
        FloatPoint startPoint(boxOrigin.x(), boxOrigin.y() - deltaY);
        TextRun run(textRenderer()->text()->characters() + start(), len(), textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride() || style->visuallyOrdered());
        
        // FIXME: Convert the document markers to float rects.
        IntRect markerRect = enclosingIntRect(font.selectionRectForText(run, startPoint, selHeight, startPosition, endPosition));
        istart = markerRect.x() - startPoint.x();
    ...
    pt->drawLineForTextChecking(FloatPoint(boxOrigin.x() + istart, boxOrigin.y() + underlineOffset), width, textCheckingLineStyleForMarkerType(marker.type));
}
...
void InlineTextBox::paintTextMatchMarker(GraphicsContext* pt, const FloatPoint& boxOrigin, const DocumentMarker& marker, RenderStyle* style, const Font& font)
{
    ...
    int sPos = max<int>(marker.startOffset - start(), (unsigned)0);
    int ePos = min<int>(marker.endOffset - start(), (unsigned)len());    
    TextRun run(textRenderer()->text()->characters() + start(), len(), textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride() || style->visuallyOrdered());
    ...
    if (renderer()->frame()->editor()->markedTextMatchesAreHighlighted()) {
        ...
        pt->clip(FloatRect(boxOrigin.x(), boxOrigin.y() - deltaY, logicalWidth(), selHeight));
...
void InlineTextBox::computeRectForReplacementMarker(const DocumentMarker& marker, RenderStyle* style, const Font& font)
{
    ...
    int sPos = max(marker.startOffset - start(), (unsigned)0);
    int ePos = min(marker.endOffset - start(), (unsigned)len());    
    TextRun run(textRenderer()->text()->characters() + start(), len(), textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride() || style->visuallyOrdered());
    IntPoint startPoint = IntPoint(x(), y);
...
void InlineTextBox::paintCompositionUnderline(GraphicsContext* ctx, const FloatPoint& boxOrigin, const CompositionUnderline& underline)
{
    ...
    ctx->drawLineForText(FloatPoint(boxOrigin.x() + istart, boxOrigin.y() + logicalHeight() - lineThickness), width, textRenderer()->document()->printing());
}
...
int InlineTextBox::offsetForPosition(float lineOffset, bool includePartialGlyphs) const
{
    ...
    int leftOffset = isLeftToRightDirection() ? 0 : len();
    int rightOffset = isLeftToRightDirection() ? len() : 0;
    ...
    int offset = f->offsetForPosition(TextRun(textRenderer()->text()->characters() + start(), len(),
        textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride() || style->visuallyOrdered()),
        lineOffset - logicalLeft(), includePartialGlyphs);
    if (blockIsInOppositeDirection && (!offset || offset == len()))
        return !offset ? len() : 0;
    return offset;
}
...
float InlineTextBox::positionForOffset(int offset) const
{
    ...
    int from = !isLeftToRightDirection() ? offset - start() : 0;
    int to = !isLeftToRightDirection() ? len() : offset - start();
    // FIXME: Do we need to add rightBearing here?
    return f.selectionRectForText(TextRun(text->text()->characters() + start(), len(), textRenderer()->allowTabs(), textPos(), expansion(), expansionBehavior(), direction(), dirOverride()),
