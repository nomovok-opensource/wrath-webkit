...
GapRects RenderBlock::selectionGaps(RenderBlock* rootBlock, const IntPoint& rootBlockPhysicalPosition, 
                                    const IntSize& offsetFromRootBlock,
                                    int& lastLogicalTop, int& lastLogicalLeft, int& lastLogicalRight, 
...
void RenderBlock::insertPositionedObject(RenderBox* o)
{
    ...
    createPositionedObjectsIfNeeded();
...
void RenderBlock::computePreferredLogicalWidths()
{
    ...
    {
        int w = computeContentBoxLogicalWidth(style()->logicalWidth().value());
        setMinPreferredLogicalWidth(w);
        setMaxPreferredLogicalWidth(w);
    }
    else {
        setMinPreferredLogicalWidth(0);
        setMaxPreferredLogicalWidth(0);
        ...
        setMaxPreferredLogicalWidth(max(getMinPreferredLogicalWidth(), getMaxPreferredLogicalWidth()));

        if (!style()->autoWrap() && childrenInline()) {
            setMinPreferredLogicalWidth(getMaxPreferredLogicalWidth());
        ...
        if (hasOverflowClip() && style()->overflowY() == OSCROLL) {
            ...
            setMaxPreferredLogicalWidth(getMaxPreferredLogicalWidth()+scrollbarWidth);
        }

        if (isTableCell()) {
            Length w = toRenderTableCell(this)->styleOrColLogicalWidth();
            if (w.isFixed() && w.value() > 0) {
                setMaxPreferredLogicalWidth(max(getMinPreferredLogicalWidth(), computeContentBoxLogicalWidth(w.value())));
                scrollbarWidth = 0;
            }
        }
        
        setMinPreferredLogicalWidth(getMinPreferredLogicalWidth()+scrollbarWidth);
    }
    
    if (style()->logicalMinWidth().isFixed() && style()->logicalMinWidth().value() > 0) {
        setMaxPreferredLogicalWidth(max(getMaxPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->logicalMinWidth().value())));
        setMinPreferredLogicalWidth(max(getMinPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->logicalMinWidth().value())));
    }
    
    if (style()->logicalMaxWidth().isFixed() && style()->logicalMaxWidth().value() != undefinedLength) {
        setMaxPreferredLogicalWidth(min(getMaxPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->logicalMaxWidth().value())));
        setMinPreferredLogicalWidth(min(getMinPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->logicalMaxWidth().value())));
    }

    int borderAndPadding = borderAndPaddingLogicalWidth();
    setMinPreferredLogicalWidth(borderAndPadding+getMinPreferredLogicalWidth());
    setMaxPreferredLogicalWidth(borderAndPadding+getMaxPreferredLogicalWidth());
        ...
        if (!child->isBR()) {
            ...
            if (!child->isRenderInline() && !child->isText()) {
                ...
                if ((canBreakReplacedElement && (autoWrap || oldAutoWrap)) || clearPreviousFloat) {
                    updateMinPreferredWidth(inlineMin);
                    inlineMin = 0;
                }

                // If we're supposed to clear the previous float, then terminate maxwidth as well.
                if (clearPreviousFloat) {
                    updateMaxPreferredWidth(inlineMax);
                    inlineMax = 0;
                }
                ...
                if (!autoWrap || !canBreakReplacedElement) {
                    if (child->isFloating())
                        updateMinPreferredWidth(childMin);
                    else
                        inlineMin += childMin;
                } else {
                    // Now check our line.
                    updateMaxPreferredWidth(childMin);
                ...
                if (hasBreak) {
                    inlineMax += beginMax;
                    updateMaxPreferredWidth(inlineMax);
                    updateMaxPreferredWidth(childMax);
                    inlineMax = endMax;
                } else
                    inlineMax += childMax;
            }
...
void RenderBlock::computeBlockPreferredLogicalWidths()
{
    ...
    while (child) {
        ...
        if (child->isFloating() || (child->isBox() && toRenderBox(child)->avoidsFloats())) {
            ...
            if (child->style()->clear() & CLEFT) {
                setMaxPreferredLogicalWidth( max(floatTotalWidth, getMaxPreferredLogicalWidth()) );
                floatLeftWidth = 0;
            }
        ...
        setMinPreferredLogicalWidth(max(w, getMinPreferredLogicalWidth()));
        
        // IE ignores tables for calculation of nowrap. Makes some sense.
        if (nowrap && !child->isTable())
            setMaxPreferredLogicalWidth(max(w, getMaxPreferredLogicalWidth()));
        ...
        if (!child->isFloating()) {
            if (child->isBox() && toRenderBox(child)->avoidsFloats()) {
                ...
                setMaxPreferredLogicalWidth(max(floatLeftWidth + floatRightWidth, getMaxPreferredLogicalWidth()));
        ...
        */
        ...
        if (document()->inQuirksMode() && child->style()->logicalWidth().isPercent() &&
            !isTableCell() && child->isTable() && getMaxPreferredLogicalWidth() < BLOCK_MAX_WIDTH) {
            RenderBlock* cb = containingBlock();
            while (!cb->isRenderView() && !cb->isTableCell())
                cb = cb->containingBlock();
            if (!cb->isTableCell())
                setMaxPreferredLogicalWidth(BLOCK_MAX_WIDTH);
        }
    ...
    setMinPreferredLogicalWidth(max(0, getMinPreferredLogicalWidth()));
    setMaxPreferredLogicalWidth(max(0, getMaxPreferredLogicalWidth()));
