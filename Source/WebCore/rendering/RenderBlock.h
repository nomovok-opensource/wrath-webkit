...
public:
    ...
    typedef Signal<RenderBlock> SignalType;
    SignalType::connection connect(const SignalType::slot_type & slot) { return m_signal.connect(slot); }

    typedef boost::signals2::signal<void (RenderBox*) > RemoveFloatSignalType;
    boost::signals2::connection connectRemoveFloat(const RemoveFloatSignalType::slot_type & slot) 
    { 
      return m_removeFloat.connect(slot);
    }
#endif
...
#if USE(WRATH)
    void readyWRATHWidgetFloats(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty, bool preservePhase = false);
    void readyWRATHWidgetContents(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty);
    void readyWRATHWidgetColumnContents(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty, bool paintFloats = false);
    void readyWRATHWidgetColumnRules(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty);
    void readyWRATHWidgetChildren(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty);
    void readyWRATHWidgetSelection(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty);
    void readyWRATHWidgetCaret(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty, CaretType);
    void readyWRATHWidgetContinuationOutlines(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH &paintInfo, int tx, int ty);
#endif
...
private:
    ...
    void updateMaxPreferredWidth(float result);
    void updateMinPreferredWidth(float result);

private:
#if USE(WRATH)
    SignalType m_signal;
    RemoveFloatSignalType m_removeFloat;
#endif
};
...
inline void RenderBlock::updateMaxPreferredWidth(float result)
{
    int snappedResult = ceilf(result);
    setMaxPreferredLogicalWidth(max(getMaxPreferredLogicalWidth(), snappedResult));
}

inline void RenderBlock::updateMinPreferredWidth(float result)
{
    int snappedResult = ceilf(result);
    setMinPreferredLogicalWidth(max(getMaxPreferredLogicalWidth(), snappedResult));
}

inline void RenderBlock::createFloatingObjectsIfNeeded()
{
    if(!m_floatingObjects)
      m_floatingObjects = adoptPtr(new FloatingObjects);
}

inline void RenderBlock::createPositionedObjectsIfNeeded()
{
    if(!m_positionedObjects)
      m_positionedObjects = adoptPtr(new PositionedObjectsListHashSet);
}

inline void RenderBlock::clearPositionedObjects()
{
    m_positionedObjects.clear();
}

inline void RenderBlock::createRareDataIfNeeded()
{
