...
void RenderBox::paintMask(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    // border-fit can adjust where we paint our border and background.  
    //If set, we snugly fit our line box descendants.  (The iChat
    // balloon layout is an example of this).
...
void RenderBox::addLayoutOverflow(const IntRect& rect)
{
    ...
    createOverflowIfNeeded(clientBox, borderBoxRect());
...
void RenderBox::addVisualOverflow(const IntRect& rect)
{
    ...
    createOverflowIfNeeded(clientBoxRect(), borderBox);
