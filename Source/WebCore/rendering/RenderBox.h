...
public:
    ...
    enum MemberChange
    {
        E_minPreferredLogicalWidth,
        E_maxPreferredLogicalWidth,
        E_frameRect,
        E_marginTop,
        E_marginBottom,
        E_marginLeft,
    ...
    typedef Signal<RenderBox> SignalType;
    ...
    void setMarginTop(int margin) { SET_VALUE_AND_EMIT(m_marginTop, margin, m_signal, E_marginTop); }
    void setMarginBottom(int margin) { SET_VALUE_AND_EMIT(m_marginBottom, margin, m_signal, E_marginBottom); }
    void setMarginLeft(int margin) { SET_VALUE_AND_EMIT(m_marginLeft, margin, m_signal, E_marginLeft); }
    void setMarginRight(int margin) { SET_VALUE_AND_EMIT(m_marginRight, margin, m_signal, E_marginRight); }
  ...
  /*
    for pushContentsClipWRATH/popContentsClipWRATH pair, the cals use
    the _SAME_ handle.. i.e.

    pushContentsClipWRATH(m_handle, args..);
    stuff();
    popContentsClipWRATH(m_handle, args..);
   */
    virtual bool pushContentsClipWRATH(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int, int);
    virtual void popContentsClipWRATH(PaintedWidgetsOfWRATHHandle&,
                                      PaintInfoOfWRATH&, PaintPhase originalPhase, int tx, int ty); 
 
    virtual void readyWRATHWidgetBoxDecorations(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty);
    virtual void readyWRATHWidgetMask(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int tx, int ty);

    void readyWRATHWidgetFillLayer(PaintedWidgetsOfWRATHHandle&, const PaintInfoOfWRATH&, const Color&, const FillLayer*, 
                                    int tx, int ty, int width, int height, 
                                    BackgroundBleedAvoidance, CompositeOperator, RenderObject* backgroundObject);
    void readyWRATHWidgetFillLayers(PaintedWidgetsOfWRATHHandle&, const PaintInfoOfWRATH&, const Color&, const FillLayer*, 
                                     int tx, int ty, int width, int height, 
                                     BackgroundBleedAvoidance = BackgroundBleedNone, 
                                     CompositeOperator = CompositeSourceOver, RenderObject* backgroundObject = 0);

    void readyWRATHWidgetBoxDecorationsWithSize(PaintedWidgetsOfWRATHHandle&, 
                                                 PaintInfoOfWRATH&, int tx, int ty, int width, int height);
    void readyWRATHWidgetMaskImages(PaintedWidgetsOfWRATHHandle&, 
                                     const PaintInfoOfWRATH&, int tx, int ty, int width, int height);
    void readyWRATHWidgetRootBoxFillLayers(PaintedWidgetsOfWRATHHandle&, 
                                            const PaintInfoOfWRATH&);
#endif
    ...
    void setInlineBoxWrapper(InlineBox* boxWrapper) { SET_VALUE_AND_EMIT(m_inlineBoxWrapper, boxWrapper, m_signal, E_inlineBoxWrapper); }
...
protected:
    void setMinPreferredLogicalWidth(int v) { SET_VALUE_AND_EMIT(m_minPreferredLogicalWidth, v, m_signal, E_minPreferredLogicalWidth); }
    void setMaxPreferredLogicalWidth(int v) { SET_VALUE_AND_EMIT(m_maxPreferredLogicalWidth, v, m_signal, E_maxPreferredLogicalWidth); }
    int getMinPreferredLogicalWidth() const { return m_minPreferredLogicalWidth; }
    int getMaxPreferredLogicalWidth() const { return m_maxPreferredLogicalWidth; }
    void incMinPreferredLogicalWidth(int v) 
    { SET_VALUE_AND_EMIT(m_minPreferredLogicalWidth, m_minPreferredLogicalWidth+v, m_signal, E_minPreferredLogicalWidth); }
    void incMaxPreferredLogicalWidth(int v) 
    { SET_VALUE_AND_EMIT(m_maxPreferredLogicalWidth, m_maxPreferredLogicalWidth+v, m_signal, E_maxPreferredLogicalWidth); }
    ...
    void createOverflowIfNeeded(const IntRect & layoutRect, const IntRect & visualRect);
    void clearOverflow();
...
inline void RenderBox::createOverflowIfNeeded(const IntRect & layoutRect, const IntRect & visualRect)
{
    if(!m_overflow)
      m_overflow = adoptPtr(new RenderOverflow(layoutRect, visualRect));
}

inline void RenderBox::clearOverflow()
{
    m_overflow.clear();
}
