...
public:
...
#if USE(WRATH)
   void readyWRATHWidgetBorder(PaintedWidgetsOfWRATHHandle& handle, 
   ...
   bool readyWRATHWidgetNinePieceImage(PaintedWidgetsOfWRATHHandle& handle, 
                                       ContextOfWRATH *wrath_context,
                                       int tx, int ty,
                                       int w, int h,
                                       const RenderStyle*,
    ...
    void readyWRATHWidgetBoxShadow(PaintedWidgetsOfWRATHHandle& handle, ContextOfWRATH *wrath_context,
                                   int tx, int ty, int w, int h, const RenderStyle*, ShadowStyle, 
                                   bool includeLogicalLeftEdge = true, bool includeLogicalRightEdge = true);

    void readyWRATHWidgetFillLayerExtended(PaintedWidgetsOfWRATHHandle& handle, const PaintInfoOfWRATH& paintInfo, 
                                           const Color&, const FillLayer*, int tx, int ty, int width, int height, 
                                           BackgroundBleedAvoidance, InlineFlowBox* = 0, 
                                           int inlineBoxWidth = 0, int inlineBoxHeight = 0, 
                                           CompositeOperator = CompositeSourceOver, 
...
private:
    ...
    void setLayer(RenderLayer * l) { SET_VALUE_AND_EMIT(m_layer, l, m_signal, E_layer); }
...
#if USE(WRATH)
protected:
    typedef Signal<RenderBoxModelObject> SignalType;
