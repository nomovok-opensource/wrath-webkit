...
void RenderEmbeddedObject::setShowsMissingPluginIndicator()
{
    ...
    setReplacementText(missingPluginText());
#if USE(WRATH)
    m_signal.setValueAndEmit(m_showsMissingPluginIndicator, true, E_showsMissingPluginIndicator);
#endif
}
...
{
    ASSERT(m_replacementText.isEmpty());
    setReplacementText(crashedPluginText());
}
...
#if USE(WRATH)
    m_signal(E_missingPluginIndicatorIsPressed);
#endif
    repaint();
}
...
bool RenderEmbeddedObject::getReplacementTextGeometry(int tx, int ty, FloatRect& contentRect, Path& path, FloatRect& replacementTextRect, Font& font, TextRun& run, float& textWidth)
{
  ...
  return_value=getReplacementTextGeometry(tx, ty, contentRect, replacementTextRect, 
                                          font, run, textWidth, pathRect);
  path.addRoundedRect(pathRect, FloatSize(replacementTextRoundedRectRadius, replacementTextRoundedRectRadius));
  return return_value;
}

bool RenderEmbeddedObject::getReplacementTextGeometry(int tx, int ty, FloatRect& contentRect, FloatRect& replacementTextRect, Font& font, TextRun& run, float& textWidth, FloatRect& pathRect)
{
...
void RenderEmbeddedObject::handleMissingPluginIndicatorEvent(Event* event)
{
    ...
    if (event->type() == eventNames().mousedownEvent && static_cast<MouseEvent*>(event)->button() == LeftButton) {
        SET_VALUE_AND_EMIT(
                m_mouseDownWasInMissingPluginIndicator, isInMissingPluginIndicator(mouseEvent), m_signal,
                E_mouseDownWasInMissingPluginIndicator);
    ...
    if (event->type() == eventNames().mouseupEvent && static_cast<MouseEvent*>(event)->button() == LeftButton) {
        ...
        SET_VALUE_AND_EMIT(m_mouseDownWasInMissingPluginIndicator, false, m_signal,
                E_mouseDownWasInMissingPluginIndicator);
        event->setDefaultHandled();
    }
