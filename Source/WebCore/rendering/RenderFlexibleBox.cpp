...
:RenderBlock(node)
{
    ...
    setFlexingChildren(false);
    setStretchingChildren(false);
}
...
void RenderFlexibleBox::calcHorizontalPrefWidths()
{
    for (RenderBox* child = firstChildBox(); child; child = child->nextSiblingBox()) {
        ...
        incMinPreferredLogicalWidth(child->minPreferredLogicalWidth() + margin);
        incMaxPreferredLogicalWidth(child->maxPreferredLogicalWidth() + margin);
    }    
}
...
void RenderFlexibleBox::computePreferredLogicalWidths()
{
    ...
    {
        int w = computeContentBoxLogicalWidth(style()->width().value());
    ...
    if (hasOverflowClip() && style()->overflowY() == OSCROLL) {
        ...
        incMaxPreferredLogicalWidth(scrollbarWidth);
        incMinPreferredLogicalWidth(scrollbarWidth);
    }

    if (style()->minWidth().isFixed() && style()->minWidth().value() > 0) {
        setMaxPreferredLogicalWidth(max(getMaxPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->minWidth().value())));
        setMinPreferredLogicalWidth(max(getMinPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->minWidth().value())));
    }
    
    if (style()->maxWidth().isFixed() && style()->maxWidth().value() != undefinedLength) {
        setMaxPreferredLogicalWidth(min(getMaxPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->maxWidth().value())));
        setMinPreferredLogicalWidth(min(getMinPreferredLogicalWidth(), computeContentBoxLogicalWidth(style()->maxWidth().value())));
    }

    int borderAndPadding = borderAndPaddingLogicalWidth();
    incMinPreferredLogicalWidth(borderAndPadding);
    incMaxPreferredLogicalWidth(borderAndPadding);
...
void RenderFlexibleBox::layoutHorizontalBox(bool relayoutChildren)
{
    ...
    do {
        ...
        setStretchingChildren(style()->boxAlign() == BSTRETCH);
        child = iterator.first();
        while (child) {
            if (child->isPositioned()) {
