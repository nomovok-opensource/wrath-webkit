...
namespace {
    inline bool changeValue(bool & v, bool t)
    {
        if(v!=t)
        {
            v=t;
...
void RenderFrameSet::fillFromEdgeInfo(const FrameEdgeInfo& edgeInfo, int r, int c)
{
    ...
    if (edgeInfo.allowBorder(LeftFrameEdge))
        colsChanged |= changeValue(m_cols.m_allowBorder[c], true);
    if (edgeInfo.allowBorder(RightFrameEdge))
        colsChanged |= changeValue(m_cols.m_allowBorder[c + 1], true);
    if (edgeInfo.preventResize(LeftFrameEdge))
        colsChanged |= changeValue(m_cols.m_preventResize[c], true);
    if (edgeInfo.preventResize(RightFrameEdge))
        colsChanged |= changeValue(m_cols.m_preventResize[c + 1], true);
#if USE(WRATH)
    if (colsChanged)
        m_signal(E_cols);
#endif
    ...
    if (edgeInfo.allowBorder(TopFrameEdge))
        rowsChanged |= changeValue(m_rows.m_allowBorder[r], true);
    if (edgeInfo.allowBorder(BottomFrameEdge))
        rowsChanged |= changeValue(m_rows.m_allowBorder[r + 1], true);
    if (edgeInfo.preventResize(TopFrameEdge))
        rowsChanged |= changeValue(m_rows.m_preventResize[r], true);
    if (edgeInfo.preventResize(BottomFrameEdge))
        rowsChanged |= changeValue(m_rows.m_preventResize[r + 1], true);
#if USE(WRATH)
    if (rowsChanged)
        m_signal(E_rows);
#endif
...
{
    SET_VALUE_AND_EMIT(m_isResizing, isResizing, m_signal, E_isResizing);
    for (RenderObject* ancestor = parent(); ancestor; ancestor = ancestor->parent()) {
        if (ancestor->isFrameSet())
        {
            RenderFrameSet * a = toRenderFrameSet(ancestor);
            SET_VALUE_AND_EMIT(a->m_isChildResizing, isResizing, m_signal, E_isChildResizing);
        }
    }
