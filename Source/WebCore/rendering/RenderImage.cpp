...
void RenderImage::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (m_needsToSetSizeForAltText) {
        ...
        setNeedsToSetSizeForAltText(false);
    }
}
...
void RenderImage::imageChanged(WrappedImagePtr newImage, const IntRect* rect)
{
    ...
    if (m_imageResource->errorOccurred()) {
        if (!m_altText.isEmpty() && document()->isPendingStyleRecalc()) {
            ASSERT(node());
            if (node()) {
                setNeedsToSetSizeForAltText(true);
...
void RenderImage::updateAltText()
{
    ...
    if (node()->hasTagName(inputTag))
        newAltText = static_cast<HTMLInputElement*>(node())->altText();
    else if (node()->hasTagName(imgTag))
        newAltText = static_cast<HTMLImageElement*>(node())->altText();
    else return;

    SET_VALUE_AND_EMIT(m_altText, newAltText, m_signal, E_altText);
}
