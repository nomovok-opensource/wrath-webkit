    ...
    , m_resizer(0)
{
    ...
    if (!renderer->firstChild() && renderer->style()) {
        setVisibleContentStatusDirty(false);
    ...
    if (cachedOffset) {
        ...
        if (disableOffsetCache)
            ...
            if (!m_parent || m_parent->renderer() == renderer()->containingBlock())
                cachedOffset->move(x(), y()); // Fast case
            else {
                int x = 0;
                int y = 0;
        ...
        if (flags & CheckForRepaint) {
            if (view && !view->printing()) {
                if (m_needsFullRepaint) {
                    renderer()->repaintUsingContainer(repaintContainer, repaintRect());
                    if (newRect != repaintRect())
                        renderer()->repaintUsingContainer(repaintContainer, newRect);
                } else
                    renderer()->repaintAfterLayoutIfNeeded(repaintContainer, repaintRect(), outlineBox(), &newRect, &newOutlineBox);
            }
        }
        setRepaintRect(newRect);
        setOutlineBox(newOutlineBox);
    } else {
        setRepaintRect(IntRect());
        setOutlineBox(IntRect());
    }
...
{
    IntRect r_repaintRect = repaintRect();
    for (RenderLayer* child = firstChild(); child; child = child->nextSibling())
        r_repaintRect.unite(child->repaintRectIncludingDescendants());
    return r_repaintRect;
}

void RenderLayer::computeRepaintRects()
{
    RenderBoxModelObject* repaintContainer = renderer()->containerForRepaint();
    setRepaintRect(renderer()->clippedOverflowRectForRepaint(repaintContainer));
    setOutlineBox(renderer()->outlineBoundsForRepaint(repaintContainer));
}
...
int RenderLayer::hierarchySize(void) const
{
  int r;

  if(transform() /*|| isTransparent()*/ || !parent()) {
    r=1;
  }
  else {
    r=0;
  ...
  return r
    + hierarchySize(m_posZOrderList)
    + hierarchySize(m_negZOrderList)
    + hierarchySize(m_normalFlowList);
}

int RenderLayer::hierarchySize(Vector<RenderLayer*>* list) const
{
  int return_value(0);

  if(list) {
    for (size_t i = 0; i < list->size(); ++i) {
      return_value+=list->at(i)->hierarchySize();
    }
  }
  return return_value;
}
...
void RenderLayer::updatePagination()
{
    ...
    if (isNormalFlowOnly()) {
        setPaginated(parent()->renderer()->hasColumns());
        return;
    }
    ...
    for (RenderLayer* curr = parent(); curr; curr = curr->parent()) {
        if (curr->renderer()->hasColumns()) {
            setPaginated(checkContainingBlockChainForPagination(renderer(), curr->renderBox()));
            return;
        }
...
{ 
    if (m_hasVisibleContent == b && !isVisibleContentStatusDirty())
        return;
    setVisibleContentStatusDirty(false);
...
{
    setVisibleContentStatusDirty(true);
    if (parent())
...
void RenderLayer::updateVisibilityStatus()
{
    ...
    if (isVisibleContentStatusDirty()) {
...
void RenderLayer::dirty3DTransformedDescendantStatus()
{
    ...
    if (curr)
        curr->set3DTransformedDescendantStatusDirty();
...
{
    if (is3DTransformedDescendantStatusDirty()) {
        setHas3DTransformedDescendant(false);
        ...
        if (m_posZOrderList) {
            ...
            {
                bool b = m_posZOrderList->at(i)->update3DTransformedDescendantStatus();
                setHas3DTransformedDescendant(has3DTransformedDescendant() | b);
            }
        }
        ...
        if (m_negZOrderList) {
            ...
            {
                bool b = m_negZOrderList->at(i)->update3DTransformedDescendantStatus();
                setHas3DTransformedDescendant(has3DTransformedDescendant() | b);
            }
        }
        
        set3DTransformedDescendantStatusDirty(false);
    }
    
    // If we live in a 3d hierarchy, then the layer at the root of that hierarchy needs
    // the m_has3DTransformedDescendant set.
    if (preserves3D())
        return has3DTransform() || has3DTransformedDescendant();
...
void RenderLayer::updateLayerPosition()
{
    ...
    if (renderer()->isRelPositioned()) {
        IntSize offset = renderer()->relativePositionOffset();
        setRelativePosition(offset);
        localPoint.move(offset);
    }
...
void RenderLayer::setLocation(int x, int y)
{
  m_x = x;
  m_y = y;
}
...
{
    if (p->paintingDisabled() || (paintsWithTransparency(paintBehavior) && isUsedTransparency()))
        return;
...
void RenderLayer::scrollTo(int x, int y)
{
    ...
    if (scrollY() == newScrollY && scrollX() == newScrollX)
        return;
...
void RenderLayer::computeScrollDimensions(bool* needHBar, bool* needVBar)
{
    ...
    setScrollDimensionsDirty(false);

    setScrollLeftOverflow(overflowLeft() - box->borderLeft());
    setScrollTopOverflow(overflowTop() - box->borderTop());

    setScrollWidth(overflowRight() - overflowLeft());
    setScrollHeight(overflowBottom() - overflowTop());
    
    m_scrollOrigin = IntPoint(-scrollLeftOverflow(), -scrollTopOverflow());

    if (needHBar)
        *needHBar = scrollWidth() > box->clientWidth();
    if (needVBar)
        *needVBar = scrollHeight() > box->clientHeight();
}

void RenderLayer::updateOverflowStatus(bool horizontalOverflow, bool verticalOverflow)
{
    if (isOverflowStatusDirty()) {
        setHorizontalOverflow(horizontalOverflow);
        setVerticalOverflow(verticalOverflow);
        setOverflowStatusDirty(false);
        return;
    }
    
    bool horizontalOverflowChanged = (isHorizontalOverflow() != horizontalOverflow);
    bool verticalOverflowChanged = (isVerticalOverflow() != verticalOverflow);
...
void RenderLayer::updateScrollInfoAfterLayout()
{
    ...
    if (scrollbarsChanged) {
        ...
        if (renderer()->style()->overflowX() == OAUTO || renderer()->style()->overflowY() == OAUTO) {
            if (isInOverflowRelayout()) {
                // Our proprietary overflow: overlay value doesn't trigger a layout.
                setInOverflowRelayout(true);
    ...
    if (m_hBar) {
        ...
        m_hBar->setProportion(clientWidth, scrollWidth());
    }
    if (m_vBar) {
        int clientHeight = box->clientHeight();
        int pageStep = max(max<int>(clientHeight * Scrollbar::minFractionToStepWhenPaging(), clientHeight - Scrollbar::maxOverlapBetweenPages()), 1);
        m_vBar->setSteps(Scrollbar::pixelsPerLineStep(), pageStep);
        m_vBar->setProportion(clientHeight, scrollHeight());
    }
                        ...
                        PaintLayerFlags paintFlags)
{
    ...
    if (haveTransparency && isUsedTransparency() && !paintingInsideReflection()) {
        p->endTransparencyLayer();
        p->restore();
        setUsedTransparency(false);
    }
}
                                       ...
                                       const HitTestingTransformState* transformState, double* zOffset)
{
    ...
    } else if (has3DTransformedDescendant()) {
...
{
    setBlockSelectionGapsBounds(IntRect());
    for (RenderLayer* child = firstChild(); child; child = child->nextSibling())
...
{
    if (!isStackingContext() || !isZOrderListsDirty())
        return;
    ...
    setZOrderListsDirty(false);
}

void RenderLayer::updateNormalFlowList()
{
    if (!isNormalFlowListDirty())
        return;
...
#if USE(ACCELERATED_COMPOSITING)
    if (compositor()->inCompositingMode()) {
        if ((isStackingContext() && isZOrderListsDirty()) || isNormalFlowListDirty())
...
{
    bool v_isNormalFlowOnly = shouldBeNormalFlowOnly();
    if (v_isNormalFlowOnly != isNormalFlowOnly()) {
        setNormalFlowOnly(v_isNormalFlowOnly);
