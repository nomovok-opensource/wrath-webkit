...
public:
  ...
  void readyWRATHWidgets(ContextOfWRATH*,
                         PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                         const IntRect& damageRect, PaintBehavior = PaintBehaviorNormal,
                         RenderObject* paintingRoot = 0);
  

#endif

    int hierarchySize(void) const;
    ...
    IntSize scrolledContentOffset() const { return IntSize(scrollXOffset() + scrollLeftOverflow(), scrollYOffset() + scrollTopOverflow()); }
...
#if USE(WRATH)
    void readyWRATHOverflowControls(ContextOfWRATH *ctx, PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                                    int tx, int ty, const IntRect& damageRect, bool paintingOverlayControls = false);

    void readyWRATHScrollCorner(ContextOfWRATH *ctx, PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                                int tx, int ty, const IntRect& damageRect);

    void readyWRATHResizer(ContextOfWRATH *ctx, PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                           int tx, int ty, const IntRect& damageRect);
#endif
...
#if USE(WRATH)
   void readyWRATHOverlayScrollbars(ContextOfWRATH *ctx,
                                    PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                                    const IntRect& damageRect, PaintBehavior, RenderObject* paintingRoot);
#endif
    ...
    void calculateRects(const RenderLayer* rootLayer, const IntRect& paintDirtyRect, IntRect& layerBounds,
                        ...
                        OverlayScrollbarSizeRelevancy v = IgnoreOverlayScrollbarSize) const;
...
protected:
    void setScrollDimensionsDirty(bool b = true) { m_scrollDimensionsDirty = b; }
    bool isScrollDimensionsDirty() const { return m_scrollDimensionsDirty; }
    void setZOrderListsDirty(bool b = true) { m_zOrderListsDirty = b; }
    bool isZOrderListsDirty() const { return m_zOrderListsDirty; }
    void setNormalFlowListDirty(bool b = true) { m_normalFlowListDirty = b; }
    bool isNormalFlowListDirty() const { return m_normalFlowListDirty; }
    void setNormalFlowOnly(bool b = true) { m_isNormalFlowOnly = b; }
    void setUsedTransparency(bool b = true) { m_usedTransparency = b; }
    bool isUsedTransparency() const { return m_usedTransparency; }
    void setInOverflowRelayout(bool b = true) { m_inOverflowRelayout = b; }
    bool isInOverflowRelayout() const { return m_inOverflowRelayout; }
    void setOverflowStatusDirty(bool b = true) { m_overflowStatusDirty = b; }
    bool isOverflowStatusDirty() const { return m_overflowStatusDirty; }
    void setHorizontalOverflow(bool b = true) { m_horizontalOverflow = b; }
    bool isHorizontalOverflow() const { return m_horizontalOverflow; }
    void setVerticalOverflow(bool b = true) { m_verticalOverflow = b; }
    bool isVerticalOverflow() const { return m_verticalOverflow; }
    void setVisibleContentStatusDirty(bool b = true) { m_visibleContentStatusDirty = b; }
    bool isVisibleContentStatusDirty() const { return m_visibleContentStatusDirty; }
    void setPaginated(bool b=true) { m_isPaginated = b; }
    void set3DTransformedDescendantStatusDirty(bool b = true) { m_3DTransformedDescendantStatusDirty = b; }
    bool is3DTransformedDescendantStatusDirty() const { return m_3DTransformedDescendantStatusDirty; }

    void setCachedOverlayScrollbarOffset(const IntPoint & p) { m_cachedOverlayScrollbarOffset = p; }
    void setBlockSelectionGapsBounds(const IntRect & r) { m_blockSelectionGapsBounds = r; }
    ...
    void setRepaintRect(const IntRect & r) { m_repaintRect = r; }
    void setOutlineBox(const IntRect & r) { m_outlineBox = r; }
    IntRect outlineBox() const { return m_outlineBox; }

    // Our current relative position offset.
    int m_relX;
    int m_relY;
    
    void setRelativePosition(int x, int y) { m_relX = x; m_relY = y; }
    void setRelativePosition(const IntSize & p) { m_relX = p.width(); m_relY = p.height(); }
    ...
    void setScroll(int x, int y) { m_scrollX = x; m_scrollY = y; }
    ...
    void setScrollLeftOverflow(int v) { m_scrollLeftOverflow = v; }
    void setScrollTopOverflow(int v) { m_scrollTopOverflow = v; }
    int scrollLeftOverflow() const { return m_scrollLeftOverflow; }
    int scrollTopOverflow() const { return m_scrollTopOverflow; }
    
    // The width/height of our scrolled area.
    int m_scrollWidth;
    int m_scrollHeight;

    void setScrollWidth(int v) { m_scrollWidth = v; }
    void setScrollHeight(int v) { m_scrollHeight = v; }
...
private:
    ...
    int hierarchySize(Vector<RenderLayer*>* list) const;

#if USE(WRATH)
     
  boost::signals2::signal<void () > m_dtor_signal;
    /*
      prepare the widgets for drawing.
      notice how similar it is to paintLayer 
     */
    void readyWRATHWidgetsLayer(PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                                RenderLayer* rootLayer, ContextOfWRATH*, 
                                const IntRect& paintDirtyRect, PaintBehavior, RenderObject* paintingRoot, 
                                OverlapTestRequestMap* = 0,
                                PaintLayerFlags=0);

    void readyWRATHWidgetsList(PaintedWidgetsOfWRATHHandleT<RenderLayer> &handle,
                               Vector<RenderLayer*>* list, 
                               RenderLayer* rootLayer, 
                               ContextOfWRATH*, const IntRect& damageRect, PaintBehavior,
                               RenderObject* paintingRoot, OverlapTestRequestMap*,
                               PaintLayerFlags);
