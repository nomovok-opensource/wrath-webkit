...
public:
    ...
    {
        if(m_notInList != notInList)
        {
            m_notInList = notInList;
#if USE(WRATH)
            m_signal(E_notInList);
#endif
        }
    }
...
private:
    ...
    void setIsValueUpToDate(bool v = true) const;

#if USE(WRATH)
protected:
    typedef Signal<RenderListItem> SignalType;
...
inline void RenderListItem::setIsValueUpToDate(bool v) const
{
    m_isValueUpToDate = v;
}
