...
void RenderListMarker::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (m_image != style()->listStyleImage()) {
        if (m_image)
            m_image->removeClient(this);
        SET_VALUE_AND_EMIT(m_image, style()->listStyleImage(), m_signal, E_image);
        if (m_image)
...
void RenderListMarker::computePreferredLogicalWidths()
{
    ...
    if (isImage()) {
        ...
        int w = style()->isHorizontalWritingMode() ? imageSize.width() : imageSize.height();
    ...
    switch (type) {
        ...
        case Asterisks:
        case Footnotes:
            setTextMember(listMarkerText(type, m_listItem->value()));
            logicalWidth = font.width(m_text); // no suffix for these types
            break;
        case Circle:
        case Disc:
        case Square:
            setTextMember(listMarkerText(type, 0)); // value is ignored for these types
    ...
    setMinPreferredLogicalWidth(logicalWidth);
    setMaxPreferredLogicalWidth(logicalWidth);
