...
void RenderMenuList::computePreferredLogicalWidths()
{
    ...
    if (style()->width().isFixed() && style()->width().value() > 0)
    {
        ...
        setMaxPreferredLogicalWidth(max(m_optionsWidth, theme()->minimumMenuListSize(style())) + m_innerBlock->paddingLeft() + m_innerBlock->paddingRight());
...
{
    m_signal.setValueAndEmit(m_popupIsVisible, false, E_popupIsVisible);
}
