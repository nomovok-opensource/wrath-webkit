...
{
#if USE(WRATH)
    if( m_preferredLogicalWidthsDirty != b )
        m_signal(E_preferredLogicalWidthsDirty);
#endif
...
void RenderObject::paintOutline(GraphicsContext* graphicsContext, int tx, int ty, int w, int h)
{
    ...
    drawLineForBoxSide(graphicsContext, leftOuter, topOuter, leftInner, bottomOuter, BSLeft,
                       outlineColor, outlineStyle, outlineWidth, outlineWidth);
    drawLineForBoxSide(graphicsContext, leftOuter, topOuter, rightOuter, topInner, BSTop,
                       outlineColor, outlineStyle, outlineWidth, outlineWidth);
    drawLineForBoxSide(graphicsContext, rightInner, topOuter, rightOuter, bottomOuter, BSRight,
                       outlineColor, outlineStyle, outlineWidth, outlineWidth);
    drawLineForBoxSide(graphicsContext, leftOuter, bottomInner, rightOuter, bottomOuter,
                       BSBottom, outlineColor, outlineStyle, outlineWidth, outlineWidth);
}
...
{
    SET_VALUE_AND_EMIT(m_style, style, m_signal, E_style);
}
...
void RenderObject::styleWillChange(StyleDifference diff, const RenderStyle* newStyle)
{
    if (m_style) {
        ...
        setHorizontalWritingMode(true); // m_horizontalWritingMode = true;
        setHasBoxDecorations(false); // m_paintBackground = false;
        setHasOverflowClip(false); // m_hasOverflowClip = false;
        setHasTransform(false); // m_hasTransform = false;
        setHasReflection(false); // m_hasReflection = false;
    } else
...
#if USE(WRATH)
    m_style_change_pre_signal(diff, newStyle);
#endif
}
...
{
#if USE(WRATH)
    m_style_change_signal(diff, oldStyle);
#endif
