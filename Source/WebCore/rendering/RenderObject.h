...
#if USE(WRATH)
  /*
    readyWRATHWidget::readyWRATHWidgetBorder follows
    that of HAVE_PATH_BASED_BORDER_RADIUS_DRAWING
    since it clips less.
   */
   #ifndef HAVE_PATH_BASED_BORDER_RADIUS_DRAWING
     #define HAVE_PATH_BASED_BORDER_RADIUS_DRAWING 1
   #endif
...
public:
    ...
    enum MemberChange
    {
        ...
        E_hasCounterNodeMap,
        E_everHadLayout,
        E_normalChildNeedsLayout,
        E_needsPositionedMovementLayout,
        E_needsSimplifiedNormalFlowLayout,
        E_hasOverrideSize,
        E_marginBeforeQuirk,
        E_marginAfterQuirk,
        E_selectionState,
        E_hasMarkupTruncation,
        E_style,
        E_hasReflection,
        E_hasTransform,
        E_hasLayer,
        E_hasOverflowClip,
        E_horizontalWritingMode,
        E_replaced,
        E_isText,
        E_isBox,
        E_paintBackground,
    ...
    typedef Signal<RenderObject> SignalType;
    ...
    typedef boost::signals2::signal<void (StyleDifference, const RenderStyle*) >::slot_type on_style_change_signal_slot_type;
    boost::signals2::connection connect_style_change(const on_style_change_signal_slot_type & s)
    {
        return m_style_change_signal.connect(s);
    }
    boost::signals2::connection connect_style_pre_change(const on_style_change_signal_slot_type & s)
    {
        return m_style_change_pre_signal.connect(s);
    }
#endif
  ...
  /*
    A derived class needs to implment:
    if it has visible elements:
    - readyWRATHWidgets
    which is called by a parent RenderObject
    or a RenderLayer to have the RenderObject
    makes it's widgets ready. The meaning of 
    readyWRATHWidgets() is very close to 
    paint(PaintInfo &, int tx, int ty)
    where the argument parent_widget holds
    the translation by (tx, ty). RenderObjects
    with child RenderObjects are expected to
    call readyWRATHWidgets as well, in the exact same
    fashion as paint()...

    The main different between paint() and readyWRATHWidgets
    is that readyWRATHWidgets first argument is a handle 
    which holds a PaintedWidgetsOfWRATHBase-pointer.
    The object pointed to by that pointer is to be a
    privately declared class to hold the widgets
    of that call. In addition, it is guaranteed that
    different paint phases of calling readyWRATHWidgets
    will be given different handles. The guarnantee must 
    be maintained when calling child object's readyWRATHWidgets,
    i.e. a distinct PaintedWidgetsOfWRATHHandle for each
    phase of each child object
   */
  virtual void readyWRATHWidgets(PaintedWidgetsOfWRATHHandle&, 
                                 PaintInfoOfWRATH &/*paintInfo*/, int /*tx*/, int /*ty*/)
  {}
 
  void readyWRATHWidgetFocusRing(PaintedWidgetsOfWRATHHandle&, ContextOfWRATH*, int tx, int ty, RenderStyle*);
  void readyWRATHWidgetOutline(PaintedWidgetsOfWRATHHandle&, ContextOfWRATH*, int tx, int ty, int w, int h);
  void readyWRATHWidgetDrawLineForBoxSide(PaintedWidgetsOfWRATHHandle&, ContextOfWRATH*, 
...
protected:
    ...
    void setPreviousSibling(RenderObject* previous) { SET_VALUE_AND_EMIT(m_previous, previous, m_signal, E_previous); }
    void setNextSibling(RenderObject* next) { SET_VALUE_AND_EMIT(m_next, next, m_signal, E_next); }
    void setParent(RenderObject* parent) { SET_VALUE_AND_EMIT(m_parent, parent, m_signal, E_parent); }
    //////////////////////////////////////////
private:
...
public:
    ...
    void setChildrenInline(bool b = true) { SET_VALUE_AND_EMIT(m_childrenInline, b, m_signal, E_childrenInline); }

    bool hasColumns() const { return m_hasColumns; }
    void setHasColumns(bool b = true) { SET_VALUE_AND_EMIT(m_hasColumns, b, m_signal, E_hasColumns); }
    ...
    void setIsAnonymous(bool b) { SET_VALUE_AND_EMIT(m_isAnonymous, b, m_signal, E_isAnonymous); }
    ...
    void setNode(Node* node) { SET_VALUE_AND_EMIT(m_node, node, m_signal, E_node); }
    ...
    void setPositioned(bool b = true) { SET_VALUE_AND_EMIT(m_positioned, b, m_signal, E_positioned); }

    void setRelPositioned(bool b = true) { SET_VALUE_AND_EMIT(m_relPositioned, b, m_signal, E_relPositioned); }

    void setFloating(bool b = true) { SET_VALUE_AND_EMIT(m_floating, b, m_signal, E_floating); }

    void setInline(bool b = true) { SET_VALUE_AND_EMIT(m_inline, b, m_signal, E_inline); }

    void setHasBoxDecorations(bool b = true) { SET_VALUE_AND_EMIT(m_paintBackground, b, m_signal, E_paintBackground); }

    void setIsText() { SET_VALUE_AND_EMIT(m_isText, true, m_signal, E_isText); }

    void setIsBox() { SET_VALUE_AND_EMIT(m_isBox, true, m_signal, E_isBox); }

    void setReplaced(bool b = true) { SET_VALUE_AND_EMIT(m_replaced, b, m_signal, E_replaced); }

    void setHorizontalWritingMode(bool b = true) { SET_VALUE_AND_EMIT(m_horizontalWritingMode, b, m_signal, E_horizontalWritingMode); }

    void setHasOverflowClip(bool b = true) { SET_VALUE_AND_EMIT(m_hasOverflowClip, b, m_signal, E_hasOverflowClip); }

    void setHasLayer(bool b = true) { SET_VALUE_AND_EMIT(m_hasLayer, b, m_signal, E_hasLayer); }

    void setHasTransform(bool b = true) { SET_VALUE_AND_EMIT(m_hasTransform, b, m_signal, E_hasTransform); }

    void setHasReflection(bool b = true) { SET_VALUE_AND_EMIT(m_hasReflection, b, m_signal, E_hasReflection); }
    ...
    void setHasMarkupTruncation(bool b = true) { SET_VALUE_AND_EMIT(m_hasMarkupTruncation, b, m_signal, E_hasMarkupTruncation); }
    ...
    virtual void setSelectionState(SelectionState state) { SET_VALUE_AND_EMIT(m_selectionState, state, m_signal, E_selectionState); }
    ...
    void setMarginBeforeQuirk(bool b = true) { SET_VALUE_AND_EMIT(m_marginBeforeQuirk, b, m_signal, E_marginBeforeQuirk); }
    void setMarginAfterQuirk(bool b = true) { SET_VALUE_AND_EMIT(m_marginAfterQuirk, b, m_signal, E_marginAfterQuirk); }
    ...
    void setHasOverrideSize(bool b) { SET_VALUE_AND_EMIT(m_hasOverrideSize, b, m_signal, E_hasOverrideSize); }
...
public:
    // Public members transformed to protected members with private accessor functions
    void setHasCounterNodeMap(bool b = true) { SET_VALUE_AND_EMIT(m_hasCounterNodeMap, b, m_signal, E_hasCounterNodeMap); }
    bool hasCounterNodeMap() const { return m_hasCounterNodeMap; }

    void setEverHadLayout(bool b = true) { SET_VALUE_AND_EMIT(m_everHadLayout, b, m_signal, E_everHadLayout); }
...
private:
    ...
    // Called before style changes, with the new style and the difference
    boost::signals2::signal<void (StyleDifference, const RenderStyle*) > m_style_change_pre_signal;
    // Called after the style changes are done, with the old style and the difference
    boost::signals2::signal<void (StyleDifference, const RenderStyle*) > m_style_change_signal;
#endif
};
...
inline void RenderObject::setNeedsLayout(bool b, bool markParents)
{
    ...
    m_needsLayout = b;
#if USE(WRATH)
    if(b != alreadyNeededLayout)
        m_signal(E_needsLayout);
#endif
    if (b) {
...
#if USE(WRATH)
    if(m_normalChildNeedsLayout != alreadyNeededLayout)
        m_signal(E_normalChildNeedsLayout);
#endif
    if (b) {
        ASSERT(!isSetNeedsLayoutForbidden());
...
{
#if USE(WRATH)
    if(!m_needsPositionedMovementLayout)
        m_signal(E_needsPositionedMovementLayout);
#endif
...
{
#if USE(WRATH)
    if(!m_needsSimplifiedNormalFlowLayout)
        m_signal(E_needsSimplifiedNormalFlowLayout);
#endif
