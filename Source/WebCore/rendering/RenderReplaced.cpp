...
void RenderReplaced::intrinsicSizeChanged()
{
    ...
    setIntrinsicSize(IntSize(scaledWidth, scaledHeight));
...
void RenderReplaced::computePreferredLogicalWidths()
{
    ...
    setMaxPreferredLogicalWidth(computeReplacedLogicalWidth(false) + borderAndPadding);

    if (style()->maxWidth().isFixed() && style()->maxWidth().value() != undefinedLength)
        setMaxPreferredLogicalWidth(min(getMaxPreferredLogicalWidth(), style()->maxWidth().value() + (style()->boxSizing() == CONTENT_BOX ? borderAndPadding : 0)));
...
void RenderReplaced::setIntrinsicSize(const IntSize& size)
{
    ...
    SET_VALUE_AND_EMIT(m_intrinsicSize, size, m_signal, E_intrinsicSize);
}
