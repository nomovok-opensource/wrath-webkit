...
void RenderTable::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    setHSpacing(collapseBorders() ? 0 : style()->horizontalBorderSpacing());
    setVSpacing(collapseBorders() ? 0 : style()->verticalBorderSpacing());
    ...
    if (child->isRenderBlock() && child->style()->display() == TABLE_CAPTION) {
        ...
        if (beforeChild && m_caption) {
            ...
            while (o && o != m_caption)
                o = o->previousSibling();
            if (!o) {
                SET_VALUE_AND_EMIT(m_caption, 0, m_signal, E_caption);
                setNeedsSectionRecalc();
            }
        }
        if (!m_caption)
        {
            SET_VALUE_AND_EMIT(m_caption, toRenderBlock(child), m_signal, E_caption);
        }
        else
            setNeedsSectionRecalc();
...
void RenderTable::computePreferredLogicalWidths()
{
    ...
    int minWidth = getMinPreferredLogicalWidth();
    int maxWidth = getMaxPreferredLogicalWidth();
    m_tableLayout->computePreferredLogicalWidths(minWidth, maxWidth);
    setMinPreferredLogicalWidth(minWidth);
    setMaxPreferredLogicalWidth(maxWidth);

    if (m_caption)
        setMinPreferredLogicalWidth(max(getMinPreferredLogicalWidth(), m_caption->minPreferredLogicalWidth()));
