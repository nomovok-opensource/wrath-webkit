...
public:
    ...
    enum MemberChange
    {
        ...
        E_firstBody,
        E_hasColElements,
        E_border, // for both m_borderstart and m_borderend
        E_needsSectionRecalc,
        E_hSpacing,
        E_vSpacing,
        E_numberOfChanges
    };
    ...
    void setNeedsSectionRecalc()
    {
        ...
        SET_VALUE_AND_EMIT(m_needsSectionRecalc, true, m_signal, E_needsSectionRecalc);
        setNeedsLayout(true);
    }

    void clearNeedsSectionRecalc() const
    {
        SET_VALUE_AND_EMIT(m_needsSectionRecalc, false, m_signal, E_needsSectionRecalc);
    }
...
private:
    ...
    void setHasColElements(bool v = true) const { SET_VALUE_AND_EMIT(m_hasColElements, v, m_signal, E_hasColElements); }

    void setHSpacing(short hSpacing) { SET_VALUE_AND_EMIT(m_hSpacing, hSpacing, m_signal, E_hSpacing); }
    void setVSpacing(short vSpacing) { SET_VALUE_AND_EMIT(m_vSpacing, vSpacing, m_signal, E_vSpacing); }
...
#if USE(WRATH)
public:
    typedef Signal<RenderTable> SignalType;
