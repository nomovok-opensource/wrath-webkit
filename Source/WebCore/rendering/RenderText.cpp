...
void RenderText::updateNeedsTranscoding()
{
    ...
    setNeedsTranscoding(fontTranscoder().needsTranscoding(style()->font().fontDescription(), encoding));
}
...
void RenderText::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (diff == StyleDifferenceLayout) {
        ...
        setKnownToHaveNoOverflowAndNoFallbackFonts(false);
    }
...
ALWAYS_INLINE float RenderText::widthFromCache(const Font& f, int start, int len, float xPos, HashSet<const SimpleFontData*>* fallbackFonts, GlyphOverflow* glyphOverflow) const
{
    ...
    /*
      [WRATH-TODO] this is silly, we need only to format the text
      according to the Font f and record that and when we draw 
      expose a method with the WRATH formatting data...
     */
...
void RenderText::computePreferredLogicalWidths(float leadWidth)
{
    ...
    if (fallbackFonts.isEmpty() && !glyphOverflow.left && !glyphOverflow.right && !glyphOverflow.top && !glyphOverflow.bottom)
        setKnownToHaveNoOverflowAndNoFallbackFonts(true);
}
...
void RenderText::computePreferredLogicalWidths(float leadWidth, HashSet<const SimpleFontData*>& fallbackFonts, GlyphOverflow& glyphOverflow)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (wordLen) {
            ...
            if (firstWord) {
                ...
                if (hasBreak)
                    setHasBreakableChar(true);
                setBeginMinWidth(hasBreak ? 0 : w);
            }
            setEndMinWidth(w);

            if (currMinWidth > m_minWidth)
                setMinWidth(currMinWidth);
            currMinWidth = 0;
            ...
            if (isNewline) { // Only set if preserveNewline was true and we saw a newline.
                if (firstLine) {
                    ...
                    leadWidth = 0;
                    if (!style()->autoWrap())
                        setBeginMinWidth(currMaxWidth);
                }

                if (currMaxWidth > m_maxWidth)
                    setMaxWidth(currMaxWidth);
                currMaxWidth = 0;
            } else {
    ...
    setMinWidth(max(currMinWidth, m_minWidth));
    setMaxWidth(max(currMaxWidth, m_maxWidth));
    ...
    if (style()->whiteSpace() == PRE) {
        if (firstLine)
            setBeginMinWidth(m_maxWidth);
        setEndMinWidth(currMaxWidth);
    }
...
void RenderText::setTextInternal(PassRefPtr<StringImpl> text)
{
    ...
    setIsAllASCII( m_text.containsOnlyASCII() );
#if USE(WRATH)
    m_signal(E_text);
#endif
}
...
void RenderText::positionLineBox(InlineBox* box)
{
    ...
    setContainsReversedText( m_containsReversedText|!s->isLeftToRightDirection() );
