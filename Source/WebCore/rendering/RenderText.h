...
public:
    ...
    enum MemberChange
    {
        ...
        E_hasEndWS,
        E_linesDirty,
        E_containsReversedText,
        E_isAllAscii,
        E_knownToHaveNoOverflowAndNoFallbackFonts,
        E_needsTranscoding,
        E_numberOfChanges
    };
#endif
...
private:
...
#if USE(WRATH)
    virtual void readyWRATHWidgets(PaintedWidgetsOfWRATHHandle&, PaintInfoOfWRATH&, int, int)
    { ASSERT_NOT_REACHED(); }
#endif
    ...
    {
        SET_VALUE_AND_EMIT(m_minWidth, v, m_signal, E_minWidth);
    }
    
    void setMaxWidth(float v)
    {
        SET_VALUE_AND_EMIT(m_maxWidth, v, m_signal, E_maxWidth);
    }
    
    void setBeginMinWidth(float v)
    {
        SET_VALUE_AND_EMIT(m_beginMinWidth, v, m_signal, E_beginMinWidth);
    }
    
    void setEndMinWidth(float v)
    {
        SET_VALUE_AND_EMIT(m_endMinWidth, v, m_signal, E_endMinWidth);
    }

    void setHasBreakableChar(bool b)
    {
        SET_VALUE_AND_EMIT(m_hasBreakableChar, b, m_signal, E_hasBreakableChar);
    }
    
    void setHasBreak(bool b)
    {
        SET_VALUE_AND_EMIT(m_hasBreak, b, m_signal, E_hasBreak);
    }
    
    void setHasTab(bool b)
    {
        SET_VALUE_AND_EMIT(m_hasTab, b, m_signal, E_hasTab);
    }
    
    void setHasBeginWS(bool b)
    {
        SET_VALUE_AND_EMIT(m_hasBeginWS, b, m_signal, E_hasBeginWS);
    }

    void setHasEndWS(bool b)
    {
        SET_VALUE_AND_EMIT(m_hasEndWS, b, m_signal, E_hasEndWS);
    }

    void setLinesDirty(bool b)
    {
        SET_VALUE_AND_EMIT(m_linesDirty, b, m_signal, E_linesDirty);
    }

    void setContainsReversedText(bool b)
    {
        SET_VALUE_AND_EMIT(m_containsReversedText, b, m_signal, E_containsReversedText);
    }

    void setIsAllASCII(bool b)
    {
        SET_VALUE_AND_EMIT(m_isAllASCII, b, m_signal, E_isAllAscii);
    }

    void setKnownToHaveNoOverflowAndNoFallbackFonts(bool b) const
    {
        SET_VALUE_AND_EMIT(m_knownToHaveNoOverflowAndNoFallbackFonts, b, m_signal, E_knownToHaveNoOverflowAndNoFallbackFonts);
    }

    void setNeedsTranscoding(bool b)
    {
        SET_VALUE_AND_EMIT(m_needsTranscoding, b, m_signal, E_needsTranscoding);
    }

    float m_minWidth; // here to minimize padding in 64-bit.

  /*
    [WRATH-TODO] we can pack the WRATH formatting information
    in the RenderText class and avoid the messy checking of
    in DrawnTextOfWRATH of seeing if things about it change, etc...
   */
    String m_text;
...
#if USE(WRATH)
protected:
    typedef Signal<RenderText> SignalType;
