...
void RenderTextControl::setInnerTextValue(const String& innerTextValue)
{
    ...
    if (value != text() || !m_innerText->hasChildNodes()) {
        ...
        SET_VALUE_AND_EMIT(m_lastChangeWasUserEdit, false, m_signal, E_lastChangeWasUserEdit);
    }
...
{
    SET_VALUE_AND_EMIT(m_lastChangeWasUserEdit, lastChangeWasUserEdit, m_signal, E_lastChangeWasUserEdit);
...
{
    SET_VALUE_AND_EMIT(m_lastChangeWasUserEdit, true, m_signal, E_lastChangeWasUserEdit);
}
...
void RenderTextControl::computePreferredLogicalWidths()
{
    ...
    if (style()->width().isFixed() && style()->width().value() > 0)
    {
        ...
        setMaxPreferredLogicalWidth(preferredContentWidth(getAvgCharWidth(family)) + m_innerText->renderBox()->paddingLeft() + m_innerText->renderBox()->paddingRight());
    }
    ...
    incMinPreferredLogicalWidth(toAdd);
    incMaxPreferredLogicalWidth(toAdd);
...
#if USE(WRATH)
    if (oldPlaceholderVisible != m_placeholderVisible)
        m_signal(E_placeholderVisible);
#endif
