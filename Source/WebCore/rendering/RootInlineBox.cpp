                                  ...
                                  InlineBox* markupBox)
{
    ...
    // truncated).
    bool foundBox = false;
    float mx = placeEllipsisBox(ltr, blockLeftEdge, blockRightEdge, ellipsisWidth, foundBox);
    ellipsisBox->setX(mx);
}
...
void RootInlineBox::adjustPosition(float dx, float dy)
{
    ...
    incLineTop(blockDirectionDelta);
    incLineBottom(blockDirectionDelta);
    incBlockLogicalHeight(blockDirectionDelta);
}
...
int RootInlineBox::alignBoxesInBlockDirection(int heightOfBlock, GlyphOverflowAndFallbackFontsMap& textBoxDataMap, VerticalPositionCache& verticalPositionCache)
{
    ...
    setBaselineType(requiresIdeographicBaseline(textBoxDataMap) ? IdeographicBaseline : AlphabeticBaseline);
    ...
    setHasAnnotations(hasAnnotationsBefore, hasAnnotationsAfter);
...
{
    int selectionTop = lineTop();

    if (m_hasAnnotationsBefore)
        selectionTop -= !renderer()->style()->isFlippedLinesWritingMode() ? computeOverAnnotationAdjustment(lineTop()) : computeUnderAnnotationAdjustment(lineTop());
...
{
    int selectionBottom = lineBottom();

    if (m_hasAnnotationsAfter)
        selectionBottom += !renderer()->style()->isFlippedLinesWritingMode() ? computeUnderAnnotationAdjustment(lineBottom()) : computeOverAnnotationAdjustment(lineBottom());
