...
public:
    ...
    LineClampValue m_lineClamp; // An Apple extension.
#if ENABLE(DASHBOARD_SUPPORT)
    Vector<StyleDashboardRegion> m_dashboardRegions;
#endif
    float m_opacity; // Whether or not we're transparent.

    DataRef<StyleFlexibleBoxData> m_flexibleBox; // Flexible box properties 
    ...
    unsigned m_userDrag : 2; // EUserDrag
    bool m_textOverflow : 1; // Whether or not lines that spill out should be truncated with "..."
    unsigned m_marginBeforeCollapse : 2; // EMarginCollapse
    unsigned m_marginAfterCollapse : 2; // EMarginCollapse
    unsigned m_matchNearestMailBlockquoteColor : 1; // EMatchNearestMailBlockquoteColor, FIXME: This property needs to be eliminated. It should never have been added.
