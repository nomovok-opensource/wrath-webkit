...
void QWebFramePrivate::renderRelativeCoords(GraphicsContext* context, QFlags<QWebFrame::RenderLayer> layers, const QRegion& clip)
{
    ...
    if (layers & QWebFrame::ContentsLayer) {
        for (int i = 0; i < vector.size(); ++i) {
            ...
            QRect rect = clipRect; //clipRect.intersected(view->frameRect());
            ...
            #if USE(WRATH)
            {
              context->clip(WebCore::IntRect(WebCore::IntPoint(0,0),
            ...
            #if USE(WRATH)
            {
              WebCore::FloatPoint smoothScroll;
              float smoothZoom;
              WebCore::IntRect rectI(rect);
              WebCore::FloatRect rectF(rectI);
              
              smoothScroll=view->getSmoothScroll();
              smoothZoom=view->getSmoothZoom();
              
              rectF.move(-smoothScroll.x(), -smoothScroll.y());
              context->translate(smoothScroll.x(), smoothScroll.y());
              
              rectF.scale(1.0f/smoothZoom);
              context->scale(WebCore::FloatSize(smoothZoom, smoothZoom));
              
              context->clip(rectF);
              rect=WebCore::IntRect(rectF);
            }
            #else
...
#if USE(ACCELERATED_COMPOSITING) && USE(TEXTURE_MAPPER)
        /*
          [WRATH-TODO]: do the composited layers need to be
          made scroll-zooom aware??
         */
        renderCompositedLayers(context, IntRect(clip.boundingRect()));
#endif
    }
...
#if USE(WRATH)
static void fireQtReadyWRATHWidgetSignal(QWebFrame *q)
{
  q->fireDirtyWRATHWidgetSignal();
}

void QWebFramePrivate::connectDirtyConnection(void)
{
  m_dirtyConnection.disconnect();

  if(frame->view())
    {
      m_dirtyConnection=frame->view()->dirtyWRATHWidgetsConnect(boost::bind(fireQtReadyWRATHWidgetSignal, q));
      
    }
}

void QWebFrame::fireDirtyWRATHWidgetSignal(void)
{
  emit dirtyWRATHWidgets();
}
...
#if defined(WTF_USE_WRATH) && WTF_USE_WRATH
    d->connectDirtyConnection();
    d->m_viewChangedConnection=d->frame->newViewConnect(boost::bind(&QWebFramePrivate::connectDirtyConnection, d));
#endif
}
...
#if defined(WTF_USE_WRATH) && WTF_USE_WRATH
    d->m_dirtyConnection.disconnect();
    d->m_viewChangedConnection.disconnect();
#endif
    delete d;
}
...
float QWebFrame::scaleFactor() const
{
  return d->frame->pageScaleFactor();
}

void QWebFrame::setScaleFactor(float factor)
{
  d->frame->scalePage(factor, d->frame->view()->scrollPosition());
}
...
bool QWebFrame::hasScrollbars(void)
{
  WebCore::FrameView* view = d->frame->view();
  return view && (view->horizontalScrollbar() || view->verticalScrollbar());
}

void QWebFrame::beginSmoothScrollZoom(void)
{
  WebCore::FrameView* view = d->frame->view();
  view->beginSmoothScrollZoom();
}
    
void QWebFrame::smoothScroll(qreal dx, qreal dy, bool adjust_for_parent_zoom)
{
  WebCore::FrameView* view = d->frame->view();
  
  /*
    walk up the parent tree, scaling dx and dy
    to the zoom factor's applied to the parents..
   */
  float scale_by(1.0f);

  if(adjust_for_parent_zoom) {
    if(view->frame()) {
      for(WebCore::Frame *f=view->frame()->tree()->parent(); f; f=f->tree()->parent()) {
        if(f->view()) {
          scale_by*=f->view()->getSmoothZoom();
        }        
      }
    }
    scale_by=1.0f/scale_by;
  }

  view->smoothScrollBy(WebCore::FloatSize(dx*scale_by, dy*scale_by) );
}

void QWebFrame::smoothZoom(qreal factor, QPointF zoomPivot)
{
  WebCore::FrameView* view = d->frame->view();
  view->smoothZoom(static_cast<float>(factor),
                   WebCore::FloatPoint(zoomPivot.x(), zoomPivot.y()) ); 
}

void QWebFrame::endSmoothScrollZoom(void)
{
  WebCore::FrameView* view = d->frame->view();
  view->endSmoothScrollZoom();
}
    
float QWebFrame::getZoomValue() const
{
  WebCore::FrameView* view = d->frame->view();
  return view->getSmoothZoom();
}

QPointF QWebFrame::getScrollValue() const
{
  WebCore::FrameView* view = d->frame->view();
  WebCore::FloatPoint p(view->getSmoothScroll());

  return QPointF(p.x(), p.y());
}

bool QWebFrame::inSmoothScrollZoom() const
{
  WebCore::FrameView* view = d->frame->view();
  return view->inSmoothScrollZoom();  
}

void QWebFrame::resetScrollZoom(void)
{
  WebCore::FrameView* view = d->frame->view();

  view->setSmoothZoomScroll(1.0f, WebCore::FloatPoint(0.0f, 0.0f));
  setScrollPosition(QPoint(0,0));
}

static void markAllFrameViewToReadyAll(WebCore::FrameView *v)
{
  v->onNextUpdateWRATHWidgetsUpdateAll();

  if(v->frame()) {
    WebCore::FrameTree *tree;
    tree=v->frame()->tree();

    for(Frame *f=tree->firstChild(); f; f=f->tree()->nextSibling()) {
      WebCore::FrameView *fv(f->view());
      if(fv) {
        markAllFrameViewToReadyAll(fv);
      }
    }
  }
}
...
void QWebFrame::readyWRATHWidgets(WebCore::ContextOfWRATH *ctx, 
                                  WebCore::PaintedWidgetsOfWRATHHandleT<WebCore::Widget> &handle,
                                  bool ready_all)
{
  WebCore::FrameView* view = d->frame->view();

  /*
    make the readyWRATHWidgets also apply to all
    child frames, we do this by walking the
    FrameTree in markAllFrameViewToReadyAll
   */
  view->updateLayoutAndStyleIfNeededRecursive();
  if(ready_all) {
    markAllFrameViewToReadyAll(view);
  }

  view->readyWRATHWidgets(ctx, handle);

  /*
    a second time seems necessary for some sites.
    which is just.. insane. Changing withing
    QWebView to draw again if m_diry_wrath_widgets
    is true does NOT fix the issue. Examples are
    www.gamespot.com (mouse over menu), www.google.com
    during typing.
   */
  view->readyWRATHWidgets(ctx, handle);
}
    ...
    , isScrollBar(false)
{
    ...
    if (img) {
#if USE(WRATH)
        NativeImagePtr nativeImage = img->nativeImageForCurrentFrame();
        QPixmap *pix = nativeImage ? nativeImage->getPixmap() : 0;
#else
