...
#if USE(WRATH)
    if (d->item) {
         WebCore::NativeImagePtr nativeImage =
           WebCore::iconDatabase().synchronousIconForPageURL(d->item->url(), WebCore::IntSize(16, 16))->nativeImageForCurrentFrame();
        return *(nativeImage->getPixmap());
    }
#else
    if (d->item)
