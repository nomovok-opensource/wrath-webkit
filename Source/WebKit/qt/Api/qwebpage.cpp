...
namespace {
  class ApplyQWebViewRotate:public WebCore::ApplyTransformationToPoint
  {
  public:
    explicit
    ApplyQWebViewRotate(const QWebView *v):
      m_v(v)
    {}

    virtual
    void
    operator()(WebCore::IntPoint &pt) const;
...
operator()(WebCore::IntPoint &pt) const
{
  if(m_v and m_v->rotate90degrees()) {

    int h(m_v->height()), w(m_v->width());
...
static ApplyQWebViewRotate GetTransformer(QWebPagePrivate *p)
{
  return (p && p->webview)?
    ApplyQWebViewRotate(p->webview):
    ApplyQWebViewRotate(0);
}


#define APPLY_TRANSFORM(X) .applyTransformation(GetTransformer(X))
#else
#define APPLY_TRANSFORM(X)
#endif
...
void QWebPagePrivate::mouseMoveEvent(T* ev)
{
    ...
    bool accepted = frame->eventHandler()->mouseMoved(PlatformMouseEvent(ev, 0) APPLY_TRANSFORM(this)  );
...
void QWebPagePrivate::mousePressEvent(T* ev)
{
    ...
    if (mev.button() != NoButton)
        accepted = frame->eventHandler()->handleMousePressEvent(mev APPLY_TRANSFORM(this) );
    ev->setAccepted(accepted);
...
void QWebPagePrivate::mouseReleaseEvent(T *ev)
{
    ...
    // ignore the event if we can't map Qt's mouse buttons to WebCore::MouseButton
    if (mev.button() != NoButton)
        accepted = frame->eventHandler()->handleMouseReleaseEvent(mev APPLY_TRANSFORM(this) );
    ev->setAccepted(accepted);
...
#if USE(WRATH)
            WebCore::IntPoint tr_pt(pos);
            GetTransformer(this)(tr_pt);
            HitTestResult result = frame->eventHandler()->hitTestResultAtPoint(frame->view()->windowToContents(tr_pt), false);
#else
...
void QWebPagePrivate::wheelEvent(T *ev)
{
    ...
    bool accepted = frame->eventHandler()->handleWheelEvent(pev APPLY_TRANSFORM(this) );
    ev->setAccepted(accepted);
}
...
bool QWebPagePrivate::touchEvent(QTouchEvent* event)
{
    ...
    return frame->eventHandler()->handleTouchEvent(PlatformTouchEvent(event) APPLY_TRANSFORM(this) );
#else
    event->ignore();
    return false;
#endif
...
*/
QWebFrame* QWebPage::frameAt(const QPoint& in_pos) const
{
#if USE(WRATH)
  WebCore::IntPoint tr_pt(in_pos);
  GetTransformer(d)(tr_pt);
  QPoint pos(tr_pt);
#else
  #define pos in_pos
#endif
...
void QWebPage::setView(QWidget* view)
{
    ...
    vv=qobject_cast<QWebView*>(view);
    d->webview=vv;
    
    if(vv && vv->rotate90degrees())
      {
        QSize sz(view->height(), view->width());
        setViewportSize(sz);
      }
    else
      {
...
bool QWebPage::swallowContextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (QWebFrame* webFrame = frameAt(event->pos())) {
      ...
      if (Scrollbar* scrollbar = frame->view()->scrollbarAtPoint(PlatformMouseEvent(event, 1) APPLY_TRANSFORM(d).pos() ))
            return scrollbar->contextMenu(PlatformMouseEvent(event, 1) APPLY_TRANSFORM(d));
    }

    WebCore::Frame* focusedFrame = d->page->focusController()->focusedOrMainFrame();
    focusedFrame->eventHandler()->sendContextMenuEvent(PlatformMouseEvent(event, 1) APPLY_TRANSFORM(d) );
