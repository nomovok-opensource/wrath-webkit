...
public:
...
#if USE(WRATH)
        , use_wrath(true)
        , m_on_next_wrath_paint_clear(false)
        , m_wrath_widgets_dirty(true)
        , m_wrath_canvas(0)
        , m_rotate_90(false)
        , m_ready_all_wrath_widgets(false)
        , m_disable_wrath_updates(false)
#endif
    {
        Q_ASSERT(view);
    }
    ...
    bool use_wrath;
    bool m_on_next_wrath_paint_clear;
    bool m_wrath_widgets_dirty;
    WebCore::ContextOfWRATH::Canvas *m_wrath_canvas;   
    WebCore::ContextOfWRATH::NodeHandle::AutoDelete m_root_widget;
    WebCore::ContextOfWRATH::NodeHandle::AutoDelete m_root_webcontents;
    WebCore::PaintedWidgetsOfWRATHHandleT<WebCore::Widget> m_wrath_widgets;

    bool m_rotate_90, m_ready_all_wrath_widgets, m_disable_wrath_updates;
    boost::signals2::signal<void () > m_dtor_signal;


    WRATHTime paintTimer;
#endif
...
#if USE(WRATH)
    if(m_wrath_canvas) {
      WRATHPhasedDelete(m_wrath_canvas);
    }
#endif
...
{
public:
  QPoint m_scroll_position;
  WRATHScaleTranslate m_display_transformation;
};

QWebViewScrollZoomValue::QWebViewScrollZoomValue(void):
  d(new QWebViewScrollZoomValuePrivate)
{}

QWebViewScrollZoomValue::QWebViewScrollZoomValue(const QWebViewScrollZoomValue &obj):
  d(new QWebViewScrollZoomValuePrivate(*obj.d))
{}

QWebViewScrollZoomValue::~QWebViewScrollZoomValue()
{
  WRATHassert(d);
  delete d;    
}
...
QWebViewScrollZoomValue::operator=(const QWebViewScrollZoomValue &obj)
{
  WRATHassert(obj.d);
  WRATHassert(d);
...
*/
#if USE(WRATH)
QWebView::QWebView(const WRATHTripleBufferEnabler::handle &tr, QWidget *parent, QGLWidget *shareWidget)
    : QGLWidget(parent, shareWidget)
{
  init(tr);
}
QWebView::QWebView(const WRATHTripleBufferEnabler::handle &tr, QWidget *parent, QGLContext *ctx)
  : QGLWidget(ctx, parent)
{
  init(tr);
}

QWebView::QWebView(const WRATHTripleBufferEnabler::handle &tr, QWidget *parent):
  QGLWidget(parent)
{
  init(tr);
}

const WRATHTripleBufferEnabler::handle& QWebView::tripleBufferEnabler(void)
{
  return d->m_tr;
}
...
{
#if USE(WRATH)
  qDebug() << "\n\nWarning: QWebView generating own triple buffer enabler";
...
QWebView::
#if USE(WRATH)
init(const WRATHTripleBufferEnabler::handle &h)
#else
init(void)
#endif
{
...
#if USE(WRATH)    
        connect(d->page->mainFrame(), SIGNAL(dirtyWRATHWidgets()),
                this, SLOT(markWRATHWidgetsDirtyForFun()) );
        connect(d->page, SIGNAL(frameCreated(QWebFrame*)), 
                this, SLOT(hookWRATHWidgets(QWebFrame*)));
#endif
...
void QWebView::setIsWRATHUpdateDisabled(bool v)
{
  d->m_disable_wrath_updates=v;
}

bool QWebView::isWRATHUpdateDisabled(void) const
{
  return d->m_disable_wrath_updates;
}


bool QWebView::rotate90degrees(void) const
{
  return d->m_rotate_90;
}

void QWebView::rotate90degrees(bool v)
{
  if(v==d->m_rotate_90)
    return;
...
bool QWebView::useWRATHToPaint(void)
{
  return d->use_wrath;
}

void QWebView::useWRATHToPaint(bool v)
{
  if(v!=d->use_wrath)
    {
      d->use_wrath=v;
...
void QWebView::clearWRATHWidgetOnNextPaint()
{
  d->m_on_next_wrath_paint_clear=true;
  markWRATHWidgetsDirty(true);
}
    ...
    if (d->page)
      {
        if(!d->m_rotate_90)
          ...
          {
            d->page->setViewportSize(QSize(e->size().height(), e->size().width()));
          }
      }
    markWRATHWidgetsDirty(); 
...
void QWebView::beginSmoothScrollZoom(void)
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();    
    frame->beginSmoothScrollZoom();
    update();
  }
}

void QWebView::smoothScroll(qreal dx, qreal dy)
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();    
    frame->smoothScroll(dx, dy);
    update();
  }
}

bool QWebView::inSmoothScrollZoom(void)
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();    
    return frame->inSmoothScrollZoom();
  }
...
void QWebView::smoothZoom(qreal factor, QPointF zoomPivot)
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();    
    frame->smoothZoom(factor, zoomPivot);
    update();
  }
}

void QWebView::endSmoothScrollZoom(void)
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();    
    frame->endSmoothScrollZoom();
    update();
  }
}
...
float QWebView::getZoomValue() const
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();    
    return frame->getZoomValue();
  }
  return 1.0f;
}

void resetScrollZoomRecursive(QWebFrame *frame)
{
  frame->resetScrollZoom();
  const QList<QWebFrame*> &frameList(frame->childFrames());
  
  for(QList<QWebFrame*>::const_iterator iter=frameList.begin(),
        end=frameList.end(); iter!=end; ++iter)
    {
      resetScrollZoomRecursive(*iter);
    }
}


void QWebView::resetScrollZoom(void)
{
  if(d->page) {
    QWebFrame *frame = d->page->mainFrame();  
    resetScrollZoomRecursive(frame);
    update();
  }
}


void QWebView::markWRATHWidgetsDirty(bool ready_all)
{
  d->m_wrath_widgets_dirty=true;
  d->m_ready_all_wrath_widgets=d->m_ready_all_wrath_widgets or ready_all;
  update();
}

void QWebView::markWRATHWidgetsDirtyForFun(void)
{
  if(d->use_wrath) {
    markWRATHWidgetsDirty(false);
  }
}

void QWebView::hookWRATHWidgets(QWebFrame *frame)
{
  connect(frame, SIGNAL(dirtyWRATHWidgets()),
          this, SLOT(markWRATHWidgetsDirtyForFun()) );
}


void QWebView::resizeGL(int width, int height)
{
  glViewport(0, 0, width, height);
}

boost::signals2::connection 
QWebView::connect_dtor(const boost::signals2::signal<void () >::slot_type &slot)
{
  return d->m_dtor_signal.connect(slot);
}

void QWebView::paintGL(void)
{
  if(d->m_disable_wrath_updates)
    {
      return;
    }
  ...
  float4x4 proj(float_orthogonal_projection_params(0, w, h, 0));
  ...
  if(!d->m_wrath_canvas)
    {
      d->m_wrath_canvas=WRATHNew WebCore::ContextOfWRATH::Canvas(d->m_tr);
    }
  
  if(d->m_on_next_wrath_paint_clear) {
    d->m_wrath_widgets.clear();
    d->m_on_next_wrath_paint_clear=false;
    d->m_wrath_widgets_dirty=true;
    d->m_ready_all_wrath_widgets=true;
  }
  
  if(d->m_wrath_widgets_dirty or !d->m_root_widget.widget())
    {
      int z(0);
      WebCore::ContextOfWRATH ctx(d->m_wrath_canvas, d->m_root_widget, z);

      {
        WebCore::ContextOfWRATH::AutoPushNode webContentsRoot(&ctx, d->m_root_webcontents);

        show_stats=true;
        d->m_wrath_widgets_dirty=false;
        frame->readyWRATHWidgets(&ctx, d->m_wrath_widgets, d->m_ready_all_wrath_widgets);
        d->m_ready_all_wrath_widgets=false;
      }
     
      /* */
      std::cout << "\nreadyWRATHWidgets( " << frame << ")"
                << "\n\t#items= " << ctx.counters().m_number_items
                << "\n\t#transparent rects=" << ctx.m_number_transparent_rects
                << "\n\t#transparent images=" << ctx.m_number_transparent_images
                << "\n\t#nodes= " << ctx.counters().m_number_nodes
                << "\n\t#canvases=" << ctx.counters().m_number_canvases
                << "\n\t#items created=" << ctx.counters().m_number_contructed_items
                << "\n\tz=" << z;

      std::cout << "\n\tCanvas pushes:";
      for(WebCore::ContextOfWRATH::LocationCounterRecord::const_iterator 
            iter=ctx.m_canvas_pushes.begin(), end=ctx.m_canvas_pushes.end();
          iter!=end; ++iter)
        {
          std::cout << "\n\t\t[" << iter->first.file()
                    << ", " << iter->first.line()
      ...
      std::cout << "\n\tUnimplementeds:";
      for(WebCore::ContextOfWRATH::NamedLocationCounterRecord::const_iterator 
            iter=ctx.m_unimplementeds.begin(), end=ctx.m_unimplementeds.end();
          iter!=end; ++iter)
        {
          std::cout << "\n\t\t[" << iter->first.file()
                    << ", " << iter->first.line()
      ...
      std::cout << "\n";
      /* */
      d->m_tr->signal_complete_simulation_frame();
      d->m_tr->signal_begin_presentation_frame();
  ...
  d->m_wrath_canvas->clear_and_draw(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT|GL_STENCIL_BUFFER_BIT,
                                    &proj, &draw_counts); 
  
  /**/
  if(show_stats)
    {
      std::cout << "\n\tDraw count=" << draw_counts.m_draw_count
                << "\n\tLayer count=" << draw_counts.m_layer_count
                << "\n\tGLSL program changes=" << draw_counts.m_program_count
                << "\n\ttexture changes=" << draw_counts.m_texture_choice_count
                << "\n";
    }
  /**/


  if(d->m_wrath_widgets_dirty)
    {
      std::cout << "\n***************************"
                << "\nStill dirty"
                << "\n***************************\n"
  ...
  /*
    The below is required because Qt's QPainter
    does not correctly initialize GL state
    for itself... can we say "fail"
  */
...
void QWebView::paintEvent(QPaintEvent *ev)
{
  ...
  elapsed_timeI=d->paintTimer.restart();
  ...
  if(elapsed_timeI>0)
    ...
    if(d->use_wrath) {
      /*
        Hackish, for Qt. Making a QPainter in the method
        paintGL does -bad- things to double buffer swapping,
        What we do is as follows:
         if using WRATH to paint, then do QGLWidget::paintEvent,
         which will find it's way to paintGL().
       */
